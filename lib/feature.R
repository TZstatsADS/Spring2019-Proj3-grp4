...
### Authors: Chengliang Tang/Tian Zheng/Charlie Chen
### Project 3


# helper function to get the value for the neighbor 8 pixels - central pixel
get_pixel <- function(All_value, Given_Row_Index, Given_Col_Index){
...
Get_Neighbor_and_Label <- function(Index, LR_Data, HR_Data, Samples){
  
  #define row and column length
  Row <- arrayInd(Samples[Index], dim(LR_Data))[1]
  Col <- arrayInd(Samples[Index], dim(LR_Data))[2]
  
  LR_up_left <- get_pixel(LR_Data, Row-1, Col-1)
  LR_left <- get_pixel(LR_Data, Row, Col-1)
  LR_bottom_left <- get_pixel(LR_Data, Row+1, Col-1)
  LR_up <- get_pixel(LR_Data, Row-1, Col)
  LR_center <- get_pixel(LR_Data, Row, Col)
  LR_bottom <- get_pixel(LR_Data, Row+1, Col)
  LR_up_right <- get_pixel(LR_Data, Row-1, Col+1)
  LR_right <- get_pixel(LR_Data, Row, Col+1)
  LR_bottom_right <- get_pixel(LR_Data, Row+1, Col+1)
  ...
  HR_1 <- get_pixel(HR_Data, 2*Row-1, 2*Col-1)
  HR_2 <- get_pixel(HR_Data, 2*Row, 2*Col-1)
  HR_3 <- get_pixel(HR_Data, 2*Row-1, 2*Col)
  HR_4 <- get_pixel(HR_Data, 2*Row, 2*Col)
...
Extract_Feature <- function(LR_Image, HR_Image, Color_Chanel, Sample_Size){
  ...
  for (index in c(1:Sample_Size)) {
    ...
    Result <- Get_Neighbor_and_Label(index, LR_image_data_chanel, HR_image_data_chanel, Sample_Points)
